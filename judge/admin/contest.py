...
class ContestAdmin(VersionAdmin):
    ...
    def make_visible(self, request, queryset):
        filter = None
        if not request.user.has_perm('judge.change_contest_visibility'):
            filter = Q(is_private=True) | Q(is_organization_private=True)
        count = queryset.filter(filter).update(is_visible=True)
    ...
    def make_hidden(self, request, queryset):
        ...
        if not request.user.has_perm('judge.change_contest_visibility'):
            filter = Q(is_private=True) | Q(is_organization_private=True)
        count = queryset.filter(filter).update(is_visible=False)
